/*
  ******************************************************************************	
	For this project, the following applies:
	*=============================================================================
  *        SYSCLK(Hz)                             | 64000000
  *-----------------------------------------------------------------------------
  *        AHB Prescaler                          | 1
...
int main(void)
{
  ...
  SystemClock_Config();
	...
	LL_GPIO_Init(GPIOA, &GPIO_Init_Struct);
		...
		// Vertical Deflection Stepper Motor 1, 2, 3, 4
	LL_GPIO_StructInit(&GPIO_Init_Struct);
	...
	// Rotating Disk Light Barrier 1 and Light Barrier 2
	LL_GPIO_StructInit(&GPIO_Init_Struct);
	...
	// Horizontal Deflection Stepper Motor 1, 2, 3, 4
	LL_GPIO_StructInit(&GPIO_Init_Struct);
	GPIO_Init_Struct.Mode = LL_GPIO_MODE_OUTPUT;
	GPIO_Init_Struct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
	...
	LL_TIM_StructInit(&TIM_Init_Struct);
	TIM_Init_Struct.Prescaler = 63999; // Timer frequency 1 kHz
	TIM_Init_Struct.Autoreload = 1; // 2 ms
...
void Move_Half()
{
	if((angle < 0.3926991 && angle >= 0) || (angle <= 6.28319 && angle > 5.8904862) || (angle >= -6.28319 && angle <= -5.8904862) || (angle <= 0 && angle > -0.3926991)) // 1. Area 
	{
		if((diode_0 + diode_1) < (diode_2 + diode_3))
		{
	...
	if((angle > 0.3926991 && angle <= 1.178097) || (angle > -5.8904862 && angle <= -5.1050881)) // 2. Area
	{
		if(diode_2 < diode_1)
		{
			Rotation_Vertical_Half_Step(1);
	...
	if((angle > 1.178097 && angle <= 1.9634954) || (angle > -5.1050881 && angle <= -4.3196899)) // 3. Area 
	{
		if((diode_0 + diode_1) < (diode_2 + diode_3))
		{
			Rotation_Vertical_Half_Step(0);
	...
	if((angle > 1.9634954 && angle <= 2.7488936) || (angle > -4.3196899 && angle <= -3.5342917)) // 4. Area
	{
		if(diode_2 < diode_1)
		{
			Rotation_Horizontal_Half_Step(1);
	...
	if((angle > 2.7488936 && angle <= 3.5342917) || (angle > -3.5342917 && angle <= -2.7488936)) // 5. Area
	{
		if((diode_0 + diode_1) < (diode_2 + diode_3))
		{
			Rotation_Horizontal_Half_Step(0);
	...
	if((angle > 3.5342917 && angle <= 4.3196899) || (angle > -2.7488936 && angle <= -1.9634954)) // 6. Area
	{
		if(diode_2 < diode_1)
		{
			Rotation_Vertical_Half_Step(0);
	...
	if((angle > 4.3196899 && angle <= 5.1050881) || (angle > -1.9634954 && angle <= -1.178097)) // 7. Area
	{
		if((diode_0 + diode_1) < (diode_2 + diode_3))
		{
			Rotation_Vertical_Half_Step(1);
	...
	if((angle > 5.1050881 && angle <= 5.8904862) || (angle > -1.178097 && angle <= -0.3926991)) // 8. Area
	{
		if(diode_2 < diode_1)
		{
			Rotation_Horizontal_Half_Step(0);
...
void TIM3_Counter_Routine()
{
	...
	else 
	{
		...
		if (timer_value != 0)
		{
			...
			if (left_to_right == 1) 
			{
				angle += rps*2*3.14159*timer_value/(100000);
			}
			if (right_to_left == 1)
			{
				angle -= rps*2*3.14159*timer_value/(100000);
